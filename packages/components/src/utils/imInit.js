...
let options = {
  SDKAppID: 1400118220, // 接入时需要将0替换为您的即时通信 IM 应用的 SDKAppID
};
// 创建 SDK 实例，`TIM.create()`方法对于同一个 `SDKAppID` 只会返回同一份实例
let tim = TIM.create(options); // SDK 实例通常用 tim 表示

const userID = sessionStorage.getItem("userID") || "";
const userSig = sessionStorage.getItem("userSig") || "";
if (userID && userSig) {
  tim.login({
    userID: userID,
    userSig: userSig,
...
let onConversationListUpdated = function (event) {
  console.log("Conversation", event.data); // 包含 Conversation 实例的数组
};
